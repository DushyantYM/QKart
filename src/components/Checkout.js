...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    ...
    try {
      ...
      console.log("inside get res.data",response.data);

      setAddresses({ ...addresses, all: response.data });
      console.log("addresses inside get",addresses)
      return response.data;
    } catch {
      enqueueSnackbar(
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const res = await axios.post(URL,req_body,{
        headers : {
          Authorization : `Bearer ${token}`
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      // TODO: CRIO_TASK_MODULE_CHECKOUT - Delete selected address from the backend and display the latest list of addresses
        const url_endpoint = config.endpoint;
        const res = await axios.delete(`${url_endpoint}/user/addresses/${addressId}`,
        {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    balance = parseInt(balance) - getTotalCartValue(items);
    ...
    if(validateRequest(items,addresses)){
       ...
       try{
          ...
          const res = await axios.post(`${url_endpoint}/cart/checkout`,body,{
            headers : { Authorization : `Bearer ${token}`}
          });
                    ...
                    addresses.all.map(({address,_id}) => (
                        ...
                        >
                          ...
                          <Button
                            variant="text"
                            startIcon={<Delete />}
                            onClick={async () => await deleteAddress(token, _id)}
                          >
                            DELETE
                          </Button>
                        </Box>
